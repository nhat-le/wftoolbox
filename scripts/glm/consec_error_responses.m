% Determine the mean responses to consecutive errors 
...
%     delaytrials = dt_all > max(dt_all) / 2;
    ...
    seltrials = delaytrials & pos_arr > 15;
    agg_trials = template.aggData(:,end-nframes-10:end,seltrials);
...
%% subsample
corrtrials = find(reward_t_all);
incorrtrials = find(~reward_t_all);

corrtrials = randsample(corrtrials, 907);
incorrtrials = randsample(incorrtrials, 1062);

idxtrials = [corrtrials incorrtrials];

agg_trials_master = agg_trials_master(:,:,idxtrials);
reward_t_all = reward_t_all(idxtrials);
reward_t1_all = reward_t1_all(idxtrials);
reward_t2_all = reward_t2_all(idxtrials);
reward_t3_all = reward_t3_all(idxtrials);
reward_t4_all = reward_t4_all(idxtrials);
reward_t5_all = reward_t5_all(idxtrials);
...
pvalsR = nan(Nareas, T);
pvalsR1 = nan(Nareas, T);
pvalsR2 = nan(Nareas, T);
pvalsR3 = nan(Nareas, T);
pvalsR4 = nan(Nareas, T);
pvalsR5 = nan(Nareas, T);
...
for areaID = 1:Nareas
    ...
    for t = 1:T
        ...
        pValsR(areaID, t) = mdl.Coefficients.pValue(2);
        pValsR1(areaID, t) = mdl.Coefficients.pValue(3);
        pValsR2(areaID, t) = mdl.Coefficients.pValue(4);
        pValsR3(areaID, t) = mdl.Coefficients.pValue(5);
        pValsR4(areaID, t) = mdl.Coefficients.pValue(6);
        pValsR5(areaID, t) = mdl.Coefficients.pValue(7);
...
for t = 1:T
    ...
    for i = 1:numel(areaid_lst)       
        areaID = areaid_lst(i);
        diff_map(brainmap == areaID) = pValsR4(i, t) < 0.05/75/7;
    end
